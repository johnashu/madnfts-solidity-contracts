...
interface IERC721 {
    ...
    /// @notice Safely transfers `tokenId` token from `from` to `to`, checking
    /// first that contract
    /// recipients
    ...
    /// @dev Usage of this method is discouraged, use {safeTransferFrom}
    /// whenever possible.
    /// @dev Emits a {Transfer} event.
    function transferFrom(address from, address to, uint256 tokenId) external;

    /// @notice Gives permission to `to` to transfer `tokenId` token to another
    /// account.
    ...
    /// @dev Operators can call {transferFrom} or {safeTransferFrom} for any
    /// token owned by the
    /// caller.
    ...
    /// @notice Returns if the `operator` is allowed to manage all of the assets
    /// of `owner`.
    ...
    /// @notice Queries EIP2981 royalty info for marketplace royalty payment
    /// enforcement.
