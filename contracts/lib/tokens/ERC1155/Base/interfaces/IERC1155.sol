...
interface IERC1155 {
    ...
    /// @dev Emitted when `account` grants or revokes permission to `operator`
    /// to
    ...
    /// @return Returns the amount of tokens of token type `id` owned by
    /// `account`.
    ...
    /// @notice Transfers `amount` tokens of token type `id` from `from` to
    /// `to`,
    ...
    /// @notice Grants or revokes permission to `operator` to transfer the
    /// caller's tokens,
    ...
    /// @notice Returns true if `operator` is approved to transfer ``account``'s
    /// tokens.
